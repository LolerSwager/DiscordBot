...
client.on("messageCreate", (message) => {
    ...
    if (message.content.includes('https://discord.com/invite/') || message.content.includes('https://discord.gg/')){
        if (message.author.id == UserArr[0]){
            ...
            message.delete()
            message.guild.channels.cache.get(message.channelId).send(`<@!${message.author.id}> You dont have Permission to Send Discord invites`)
            nodeConsole("info", "premission denied", `${message.author.username} - tryed to Send Discord invites ${message}`)
        }
    }
    ...
    if (message.content.includes('https://') || message.content.includes('http://')){
        if (message.content.split("/")[2].split(".").pop() == "xyz"){
            message.delete()
            client.users.cache.get(message.author.id).send(`<@!${message.author.id}> You cant send links with "xyz" do to security 🚩`)
            nodeConsole("info", "premission denied", `${message.author.username} tryed to Send "xyz link" ${message}`)
        }
    }
    ...
    if (message.content == "/restart"){
        if (message.author.id == UserArr[0]) {
            ...
            nodeConsole("info", "restarting", `${message.author.username} has restarted the ${client.user.tag}`)
            client.destroy()
            client.login(process.env.token)
            nodeConsole("ok", "starting", `${client.user.tag}`)
        }else{
            message.react('⛔')
            message.guild.channels.cache.get(message.channelId).send(`<@!${message.author.id}> You dont have Permission to use this command`)
            nodeConsole("info", "premission denied", `${message.author.username} tryed to restart the ${client.user.tag}`)
        }
    }
    ...
    if (message.content == "/stop sdk"){
        if (message.author.id == UserArr[0]) {
            ...
            nodeConsole("info", "stoped", `${message.author.username} has stoped the ${client.user.tag}`)
            client.destroy()
        }else{
            message.react('⛔')
            message.reply(` You dont have permission to use this command`)
            nodeConsole("info", "premission denied", `${message.author.username} tryed to stop the ${client.user.tag}`)
        }
    }
})
